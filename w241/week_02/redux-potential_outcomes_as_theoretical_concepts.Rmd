...
d <- data.table( id = 1:NROWS,tau=rnorm(NROWS,mean=2,sd=2)
...
d[,Age:= sample(c('young','old'),size=.N,replace = T)]
```
...
```{r}
d[ , y_0 := ifelse(Age=='young',rnorm(.N,mean=10,sd=1),rnorm(.N,mean=15,sd=1))]
```
