...
```{r}
ITT <- d[ASSIGNED==1,mean(VOTED)]-d[ASSIGNED==0,mean(VOTED)]
ITT
```
...
```{r}
ITT_d <- d[ASSIGNED == 1,mean(TREATED)]
ITT_d
```
...
```{r}
model1 <- d[,lm(VOTED~ASSIGNED)]
model2 <- d[,lm(TREATED~ASSIGNED)]
CACE_Model <- model1$coefficients["ASSIGNED"]/model2$coefficients["ASSIGNED"]
CACE_Model
Ivreg_model <- d[,ivreg(VOTED~TREATED,~ASSIGNED)]
summary(Ivreg_model)
stargazer(model1,model2,Ivreg_model,type="text")
```