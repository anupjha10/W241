...
cheat_experiment <- function(experiment_size, number_of_experiments, crit_val) { 
  ...
  for(sim in 1:number_of_experiments) { 
    ...
    y1 <- rnorm(experiment_size,mean=0,sd=1000)
    y0 <- rnorm(experiment_size,mean=0,sd=1000)
    ...
    for(i in  4:experiment_size) {
      ...
      p_val[i] <- t.test(rnorm(i),rnorm(i))$p.value
    }
    print(mean(p_val < crit_val,na.rm=TRUE))
...
```{r}
false_rejections <- cheat_experiment(500, 10, 0.05)
