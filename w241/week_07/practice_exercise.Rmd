...
```{r}
#class(d)
smallModel <- d1[, lm(tip ~ female + happyface)]
longModel <- d1[,lm(tip ~ female +happyface + female*happyface)]
shortestModel <- d1[,lm(tip ~ 1)]
femaleModel <- d1[,lm(tip~female)]
summary(femaleModel)
summary(smallModel)
summary(longModel)
summary(shortestModel)

#lm(tip~ female + happyface,data=d)
```
```{r}
anova(shortestModel,smallModel,longModel,test='F')
```

```{r}
d1[,mean(tip),by=.(female,happyface)]
```